...
export class PostsService {
    ...
    async create(createPostDto: CreatePostDto, userId: Types.ObjectId): Promise<Post> {
        const postData = { ...createPostDto, userId, isPublished: true };
    ...
    async createWithImage(createPostDto: CreatePostDto, userId: Types.ObjectId, image: Express.Multer.File): Promise<Post> {
        ...
        const postData = { ...createPostDto, userId, imageUrl, isPublished: true };
        const createdPost = new this.postModel(postData);
        return await createdPost.save();
    }
    ...
    async findAll(page: number = 1, limit: number = 10, sortBy: 'date' | 'likes' = 'date'): Promise<{ posts: Post[]; total: number }> {
        ...
        const sortOptions: { [key: string]: SortOrder } = sortBy === 'likes' ? { likes: -1 as SortOrder } : { createdAt: -1 as SortOrder };
    ...
    async adminRemove(postId: Types.ObjectId): Promise<void> {
        const result = await this.postModel.updateOne(
            { _id: postId },
            { isPublished: false }
        );
